...
<body class="min-h-screen flex flex-col">
    ...
    <main class="container mx-auto p-4 flex-grow">
        ...
        case 'dashboard': ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-gray-800 border border-gray-700 p-4 rounded-lg shadow-lg flex items-center justify-between">
                        <span class="text-gray-400 text-sm font-bold">Problem z banerem?</span>
                        <form method="POST">
                            <input type="hidden" name="refresh_banner_trigger" value="1">
                            <button type="submit" class="bg-indigo-700 hover:bg-indigo-600 text-white text-xs font-bold py-2 px-4 rounded shadow border border-indigo-500">🔄 <?= t('refresh_banner') ?></button>
                        </form>
                    </div>
                </div>
                                ...
                                <div class="w-full md:w-1/2">
                                    ...
                                    <select name="winners[<?= $idx ?>]" class="w-full bg-gray-800 text-white border <?= $suggestedWinnerId?'border-green-600':'border-gray-500' ?> rounded p-2 focus:border-yellow-500">
                                        ...
                                        <?php foreach($selectablePlayers as $pid=>$nick): ?>
                                            <option value="<?= $pid ?>" <?= $pid==$suggestedWinnerId?'selected':'' ?>><?= htmlspecialchars($nick) ?> <?= $pid==$suggestedWinnerId?'(Suggested)':'' ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
        ...
        case 'history': 
            $sessions = $pdo->query("SELECT id, name, boss, created_at FROM sessions ORDER BY created_at DESC LIMIT 50")->fetchAll(); ?>
            <div class="bg-gray-800 border border-gray-700 rounded-lg shadow-lg overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-400">
    ...
    function copyLootToClipboard() {
        ...
        rows.forEach(row => {
            ...
            if (select) {
                ...
                playerNick = rawText.replace('(Suggested)', '').trim();
            }
            textToCopy += itemName + " - " + playerNick + "\n\n";
            foundAny = true;
        });
        if (!foundAny) { alert("Brak przedmiotów do skopiowania!"); return; }
        runCopyCommand(textToCopy, document.getElementById('copy-btn'));
    }

    function copySessionLoot(btn, textToCopy) {
        if (!textToCopy) { alert('Ta sesja nie ma dropu!'); return; }
